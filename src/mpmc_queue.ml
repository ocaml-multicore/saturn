...
let push {array; tail; head; mask; _} item = 
  ...
  (* spin 30x *)
  let i = ref 0 in
  while not (Atomic.compare_and_set cell None (Some item)) && !i < spin_threshold do
    while Option.is_some (Atomic.get cell) && !i < spin_threshold do 
      i := !i + 1
    done     
  done;
  (* define clean up function *)
  let rec take_or_rollback () = 
    if Option.is_none (Atomic.get cell)
      && Atomic.compare_and_set cell None (Some item) 
    then 
      (* succedded to push *)
      true 
    else if 
      Atomic.compare_and_set tail (tail_val+1) tail_val 
    then
      (* rolled back tail *)
      false 
    else if 
      Atomic.compare_and_set head (tail_val) (tail_val+1) 
    then
      (* pushed forward head *)
      false
    else
