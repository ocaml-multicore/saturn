(** Lock-free single-producer, multi-consumer dynamic-size double-ended queue
    (deque).
    ...
    different sides, reduce contention in work distribution. Further, the local
    LIFO order, running related tasks one after another, improves locality.
...
val push : 'a t -> 'a -> unit
(** [push queue element] adds [element] at the end of the [queue]. It should
    only be invoked by the domain that owns the [queue]. *)
...
(** [pop_opt queue] removes and returns [Some] of the last element of the
    ...
    invoked by the domain that owns the [queue]. *)

val drop_exn : 'a t -> unit
(** [drop_exn queue] removes the last element of the [queue]. It should only be
    invoked by the domain that owns the [queue].
