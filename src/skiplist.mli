(** Lock-free non-resizable skiplist.
    ...
    {b Warning}: This skiplist is not resizable. It will, however, continue to
    work once the limit capacity is reached, but performance will decrease as
    the depth of the structure won't be enough to maintain logarithmic
    performance.

    {b Sources}: This implementation is inspired by the algorithm in chapter 14
    of {i The Art of Multiprocessor Programming} book. *)
...
(** [create ~compare ()] creates a new empty skiplist where keys are ordered
    ...
    current implementation does not adjust height automatically. [max_height]
    can be more than 30. *)
...
(** [find_exn sl key] returns the current binding of [key] in the skiplist [sl]
...
(** [try_add sk key value] tries to add a new binding of [key] to [value] to the
    skiplist [sl]. Returns [true] on success and [false] if the skiplist already
    contains a binding for [key]. *)

val try_remove : ('k, 'v) t -> 'k -> bool
(** [try_remove sl key] tries to remove a binding of [key] from the skiplist
    [sl]. Returns [true] on success and [false] if the skiplist does not contain
    a binding for [key]. *)
