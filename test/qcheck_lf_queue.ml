      ...
      (pair (list int) (pair small_nat small_nat)) (fun (lpush, (npop, when_close)) ->
          ...
          let expected =
            List.init npop (fun i ->
                ...
                else failwith "should not happen") in
          expected = popped);
          ...
          if npop <= List.length lpush1 then
            (
              let expected_final =
                ...
                |> list_some in
              let expected_popped =
                keep_n_first npop (List.rev lpush1)
                |> list_some in
              List.rev final = expected_final
              && popped = expected_popped)
          else
          (
            let expected_popped =
              (List.rev lpush1 |> list_some) @
              List.init (npop - List.length lpush1) (fun _ -> `None) in

            List.rev final = list_some lpush2
            && popped = expected_popped));
  ...
  QCheck.([
      ...
      Test.make
        ...
        (pair (list int) small_nat) (fun (lpush, npop) ->
            ...
            (* Testing property *)
            let expected =
              (keep_n_first npop lpush
              |> list_some) @
              (List.init (Int.max 0 (npop - (List.length lpush)))  (fun _ -> `None)) in
            popped = expected);
      ...
      Test.make
        ...
        (pair (list int) small_nat) (fun (lpush, npop) ->
            ...
            let popped_value = List.filter (function `Some _ -> true | _ -> false) popped in
            ...
            &&
            popped_value = (keep_n_first (List.length popped_value) lpush |> list_some)
            &&
            (List.for_all (function `Some _ | `None -> true | `Closed -> false)) popped);
  ...
  QCheck.([
      ...
      Test.make
        ...
        (pair (list int) (list int)) (fun (lpush1, lpush2) ->
            ...
            let popped_value =
              List.fold_left
                (fun acc elt -> match elt with Some elt -> elt::acc | _ -> acc) [] popped
              |> List.rev in
