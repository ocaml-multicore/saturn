...
let simple_two_domains_remove_add () =
  Atomic.trace (fun () ->
      Random.init 0;
      let ll = Htbl.create ~size_exponent:4 in
      ...
      Atomic.spawn (fun () -> removed1 := Htbl.try_remove ll 2);

      (* Paralled add *)
      let removed2 = ref false in
      Atomic.spawn (fun () ->
          Htbl.add ll 2 3;
          removed2 := Htbl.try_remove ll 2);
...
let two_domains_remove_add () =
  Atomic.trace (fun () ->
      Random.init 0;
      let ll = Htbl.create ~size_exponent:5 in
      ...
      (* Sequential add *)
      List.iter (fun elt -> Htbl.add ll elt 0) items_add_seq;
      ...
      Atomic.spawn (fun () ->
          removed :=
            List.fold_left
              (fun acc elt ->
                if Htbl.try_remove ll elt then elt :: acc else acc)
              [] items_remove_par);

      (* Paralled add *)
      Atomic.spawn (fun () ->
          List.iter (fun elt -> Htbl.add ll elt 1) items_add_par);
          ...
          Atomic.check (fun () ->
              ...
              Sint.for_all
                (fun elt ->
                  ...
                  with
                  | true, true, false ->
                      Htbl.mem ll elt && Htbl.find_all ll elt = [ 1; 0 ]
                  | true, false, false ->
                      Htbl.mem ll elt && Htbl.find_all ll elt = [ 0 ]
                  | false, true, false ->
                      Htbl.mem ll elt && Htbl.find_all ll elt = [ 1 ]
                  | false, false, true ->
                      (not @@ Htbl.mem ll elt) && (not @@ Sint.mem elt removed)
                  | true, false, true ->
                      (not @@ Htbl.mem ll elt) && Sint.mem elt removed
                  | false, true, true ->
                      xor (Htbl.mem ll elt) (Sint.mem elt removed)
                  | true, true, true ->
                      Sint.mem elt removed && Htbl.mem ll elt
                      &&
                      let r = Htbl.find_all ll elt in
                      r = [ 0 ] || r = [ 1 ]
                  | _ -> false)
...
let two_domains_add_add () =
  Atomic.trace (fun () ->
      ...
      Atomic.spawn (fun () ->
          List.iter (fun elt -> Htbl.add ll elt 1) items_add1_par);

      (* Domain 2 : Paralled add *)
      Atomic.spawn (fun () ->
          List.iter (fun elt -> Htbl.add ll elt 2) items_add2_par);
                  ...
                  && begin
                       match
                         ( Sint.mem elt add_seq,
                           ...
                           Sint.mem elt add2 )
                       with
                       | true, false, false -> Htbl.find_all ll elt = [ 0 ]
                       | true, true, false -> Htbl.find_all ll elt = [ 1; 0 ]
                       | true, false, true -> Htbl.find_all ll elt = [ 2; 0 ]
                       | true, true, true ->
                           let b = Htbl.find_all ll elt in
                           b = [ 2; 1; 0 ] || b = [ 1; 2; 0 ]
                       | false, true, false -> Htbl.find_all ll elt = [ 1 ]
                       | false, false, true -> Htbl.find_all ll elt = [ 2 ]
                       | false, true, true ->
                           let b = Htbl.find_all ll elt in
                           b = [ 2; 1 ] || b = [ 1; 2 ]
                       | _ -> false
...
let two_domains_add_add_smaller_htbl () =
  Atomic.trace (fun () ->
      Random.init 0;
      let ll = Htbl.create ~size_exponent:3 in
      let nadd_seq = nitems in
      let factor = 3 in
      let items_add_seq = random_uniq_list nadd_seq factor in
          ...
          Atomic.check (fun () ->
              ...
              Sint.for_all
                (fun elt ->
                  ...
                  let len_bindings = Htbl.find_all ll elt |> List.length in
                  match
                    ( Sint.mem elt add_seq,
                  ...
                  with
                  | true, false, false ->
                      Htbl.mem ll elt && len_bindings = Option.get in_add
    ...
    [
      ( "basic",
        [
          ...
          test_case "2-domains_add_add_smaller_htbl" `Slow
            two_domains_add_add_smaller_htbl;
