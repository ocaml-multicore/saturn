...
let create_output ~time_median ~throughput_median ~throughput_stddev =
  ...
  let name =
    Printf.sprintf "mpmc-queue-pushers:%d,takers:%d,use-cas:%b" !num_of_pushers !num_of_takers !use_cas_intf
  in
...
let run_bench () =
  ...
  let throughputs = List.map (fun time -> (Int.to_float !num_of_elements) /. time) times in
